                ...
                success:function(data){
                    ...
                    var tbl = $('#datatable3').dataTable({
                        ...
                        columns:[
                            ...
                            {'data':'brandName'},
                            {'data':'categoryName'},
                            {'data':'new','render': function ( data, type, full, meta ) { return data ? "Yes" : "No";}  },
                            {'data':'offer','render': function ( data, type, full, meta ) { return data ? "Yes" : "No";} },
                            {'data':'price','render': function ( data, type, full, meta ) { return "$ "+data ;}},
                            // {'data':'description'},
                            {   'data':null,
                                    ...
                                    myDropzone.on('sending', function (data, xhr, formData) {
                                     ...
                                     myDropzone.on("queuecomplete", function(file, res) {
                                            if ( myDropzone.files[0].status != Dropzone.SUCCESS ) {
                                                ...
                                                toastr.success("Item images uploaded successfully", 'Notification', {timeOut: 5000});
                                                $('#newFormContainer').hide(700);
                                                myDropzone.removeAllFiles(true); 
                                                $('#newform').find("input[type=text],input[type=file],select, textarea").val("")
                                ...
                                var itemId_= $('#editFormContainer').find('#itemName').val()
                                 $.get('../php/itemimages/get.php?itemName='+itemId_, function(data) {
                                        ...
                                        $.each(data, function(key,value){
                                            var mockFile = { name: value.imageName,imageId:value.ImageId};
                            ...
                            " style='cursor:pointer;color:#C5350A;' image-id="+file.imageId+">Delete <span style='cursor:pointer;' class='fa fa-cloud'></span></button>");
                             ...
                             removeButtonForServer.addEventListener("click", function(e) {
                                 ...
                                 $.ajax({
                                        ...
                                        success:function(data){ 
                                            toastr.success('Image deleted successfully', 'Notification', {timeOut: 5000})
                                            _this.removeFile(file);
                                        } ,
                                        error:function(){
                                            toastr.error('Could not delete this image', 'Notification', {timeOut: 5000})
                                        }
                                    })
                            });
